...
4. **Structured Agent Output**: The LLM is provided with recent events, the current order context, and rolling memory. It outputs a strictly validated JSON structure determining whether to `ACT`, `SLEEP`, or take `NO_ACTION`, along with specific business actions to execute.
5. **Mock Actions**: The business actions are implemented as Temporal Activities. They record a database entry in the `activities` table which is immediately visible in the Next.js UI timeline.
6. **Graceful Completion**: The workflow recognizes terminal events like `delivered`. The LLM does not kill the workflow; the workflow kills itself and generates a final summary activity before completing.
